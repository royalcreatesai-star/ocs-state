...
- **Residual:** the OAuth consent screen is in Google's "Testing" publishing status (100-test-user cap, not yet verified) — fine for single-operator use; revisit only if this script's audience ever grows. The original downloaded `client_secret_*.json` also sits in `~/Downloads/` — Royal to delete/secure that copy since the in-repo `harness/scripts/credentials.json` is now the working copy.
